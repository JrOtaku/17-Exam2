...
class Triangle(object):
    ...
    def shrink_or_expand(self, f):
        ...
        self.b.y *= f
        self.c.y *= f
        self.history += [self.area()]
        # self.history += [Triangle(Point(self.a.x*f, self.a.y*f),
        #                           Point(self.b.x*f, self.b.y*f),
        #                           Point(self.c.x*f, self.c.y*f)).area()]
        # ---------------------------------------------------------------------
        # Done 8:
        #   a. READ the above specification, including the Example.
        #        ** ASK QUESTIONS AS NEEDED. **
    ...
    def get_largest_area(self):
        ...
        if len(self.history) > 0:
            for k in range(len(self.history)):
                if count < self.history[k]:
                    count = self.history[k]
        else:
            count = self.area()
        return count
        # ---------------------------------------------------------------------
